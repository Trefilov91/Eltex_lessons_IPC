...
user_queue queue;
pthread_mutex_t get_user_fd;
pthread_t used_thread_check;

void *thread_echo_work(void *args)
{
    int user_fd, user_fd_index;
    ...
    while(1) {
        pthread_mutex_lock(&get_user_fd);
        *(int *)args = -2;
        while(1) {
            if(queue.read_index != queue.write_index) {
                user_fd_index = queue.read_index;
                user_fd = queue.user_fd[queue.read_index];
                if(++queue.read_index >= USER_QUEUE_LEN)
                    queue.read_index = 0;
                if(user_fd >= 0) { 
                    *(int *)args = user_fd;  
                    pthread_mutex_unlock(&get_user_fd);                             
                    break; 
                } 
            }          
        }
...
void *thread_check_used(void *args)
{
    int i;
    while(1) {
        ...
        current_user = last_user;
        while(i > START_WORK_THREADS_NUM && current_user) {
...
void *thread_echo_listener(void *args)
{
    ...
    queue.read_index = 0;
    queue.write_index = 0;
    memset(&queue.user_fd, -1, sizeof(queue.user_fd));    
    ...
    pthread_create(&used_thread_check, NULL, thread_check_used, NULL); 

    while(1) {
        while (queue.user_fd[queue.write_index] != -1)
        {
            if(++queue.write_index >= USER_QUEUE_LEN)
                queue.write_index = 0;
        }        

        client_addr_size = sizeof(client);
        queue.user_fd[queue.write_index] = accept(passiv_fd, (struct sockaddr *)&client, &client_addr_size);
        if(queue.user_fd[queue.write_index] == -1) {
            perror("accept");
            close(passiv_fd);
            exit(EXIT_FAILURE);        
        }   
...
int main()
{
    ...
    pthread_cancel(used_thread_check);
    ...
    for(i = 0; i < USER_QUEUE_LEN; i++) {
        close(queue.user_fd[i]);
    }
