...
void *thread_echo_listener(void *args)
{
    ...
    while(1) {
        ...
        last_user->user_fd = 0;
        last_user->user_thread = 0;

        client_addr_size = sizeof(client);
        last_user->user_fd = accept(passiv_fd, (struct sockaddr *)&client, &client_addr_size);
        if(last_user->user_fd == -1) {
            perror("accept");
            close(passiv_fd);
            exit(EXIT_FAILURE);        
        }   
        ...
        while(current_user) {
            if(current_user->user_fd < 0) {
                pthread_join(current_user->user_thread, (void **)&thread_status);
                current_user = del_user_from_list(current_user);
            }
            else
...
int main()
{
    ...
    while(last_user) {
        if(last_user->user_fd < 0)
            pthread_join(last_user->user_thread, (void **)&thread_status);
        else {
